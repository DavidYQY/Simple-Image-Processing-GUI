        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (positive.Checked)
            {
                if (Nx > 20 || Ny > 20)
                {
                    ...
                    MessageBox.Show("将采取Nx=20,Ny=20");
                    Nx = 20; Ny = 20;
                    m_Nx.Text = "20";
                    m_Ny.Text = "20";
                }
            ...
            previousImage = new Bitmap(myPic.Image);
            myImage = previousImage;
            Point m_Point = new Point(initital.X / Nx, initital.Y / Ny);
           // Console.WriteLine(initital.X + " " + m_Point.X * Nx);
            if (initital.X - m_Point.X * Nx > 0.5 * Nx)
            {
                initital.X = (m_Point.X + 1) * Nx;
                m_Point.X = m_Point.X + 1;
            }
            else
                initital.X = m_Point.X * Nx;
            if (initital.Y - m_Point.Y * Ny > 0.5 * Ny)
            {
                initital.Y = (m_Point.Y + 1) * Ny;
                m_Point.Y = m_Point.Y + 1;
            }
                
            else
                initital.Y = (m_Point.Y) * Ny;
            ...
            double C = 2.5;
            int R = Math.Min(Nx, Ny);
            if (r > C*R)
            {
                dx = C * Math.Cos(angle)*R;
                dy = C * Math.Sin(angle)*R;
            ...
            if (positive.Checked)//采取正变换法
            {
                ...
                for (int x0 = -2 * Nx; x0 < 2 * Nx; x0++)
                {
                    ...
                    for (int y0 = -2 * Ny; y0 < 2 * Ny; y0++)
                    {
                        ...
                        int y = y0 + initital.Y;
                        if (x < 0) x = 0;
                        if (x >= previousImage.Width) x = previousImage.Width - 1;
                        if (y < 0) y = 0;
                        if (y >= previousImage.Height) y = previousImage.Height - 1;
                        Color col;
                        ...
                        for (int i = 0; i < 4 * Nx; i++)
                            for (int j = 0; j < 4 * Ny; j++)
                            {
                                ...
                                if (dis < disMin)
                                {
                                    ...
                                    if (ansX < 0) ansY = 0;
                                    if (ansX >= previousImage.Width) ansX = previousImage.Width - 1;
                                    ansY = j - 2 * Ny + initital.Y;
                                    if (ansY < 0) ansY = 0;
                                    if (ansY >= previousImage.Height) ansY = previousImage.Height - 1;
                                    disMin = dis;
                                }
                            }
                        //Console.WriteLine("(" + x + " " + y + ")" + "(" + ansX + " " + ansY + ")");
                        col = previousImage.GetPixel(ansX, ansY);
                        //col = Color.Green;
            ...
            if (negative.Checked)
            {
                ...
                for(int x0=-2*Nx; x0<2*Nx; x0++)
                {
                    ...
                    for (int y0=-2*Ny;y0<2*Ny;y0++)
                    {
                        int x = x0 + initital.X;
                        if (x < 0) x = 0;
                        if (x >= previousImage.Width - 1) x = previousImage.Width - 1;
                        int y = y0 + initital.Y;
                        if (y < 0) y = 0;
                        if (y > previousImage.Height - 1) y = previousImage.Height - 1;
                        //Console.WriteLine(x + " " + y);
                        int i = x / Nx - 1;
                        int j = y / Ny - 1;
                        double u = (double)x / Nx - x / Nx;
